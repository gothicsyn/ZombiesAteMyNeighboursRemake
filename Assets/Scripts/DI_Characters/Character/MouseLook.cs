	...
	void Update ()
	{
		if (axes == RotationAxes.MouseXAndY) {
			...
			if (!usingController) {
				...
				rotationX = transform.localEulerAngles.y + Input.GetAxis ("Horizontal Rotation") * sensitivityX;
				rotationY += Input.GetAxis ("Vertical Rotation") * sensitivityY;
			}
			...
			if (!usingController) {
				...
				transform.Rotate (0, Input.GetAxis ("Horizontal Rotation") * sensitivityX, 0);
			}
		}
		else {
			if (!usingController) {
